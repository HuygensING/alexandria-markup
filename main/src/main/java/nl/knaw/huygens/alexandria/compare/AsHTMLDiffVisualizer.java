...
public class AsHTMLDiffVisualizer implements DiffVisualizer {
  ...
  public void startVisualization() {
    resultBuilder
        ...
        .append(
            "table table, table table th, table table td {border: 0px; border-collapse: collapse; padding: 0;}")
        .append("</style>\n")
  ...
  public void originalTextNode(final TAGTextNode t) {
    ...
    originalBuilder
        .append("    <td class=\"original\" colspan=\"original")
        .append(textNodeId)
        .append("\">")
  ...
  public void editedTextNode(final TAGTextNode t) {
    Long textNodeId = t.getDbId();
    editedBuilder
        .append("    <td class=\"edited\" colspan=\"edited")
        .append(textNodeId)
        .append("\">")
        .append(escapedText(t.getText()))
        .append("</td>\n");
  ...
  public void endVisualization() {
    ...
    for (long k : originalColSpan.keySet()) {
      original = original.replace("original" + k + "\"", originalColSpan.get(k) + "\"");
    }

    String editedMarkup = getEditedMarkup();
    String edited = editedBuilder.toString();
    for (long k : editedColSpan.keySet()) {
      edited = edited.replace("edited" + k + "\"", editedColSpan.get(k) + "\"");
    }
    resultBuilder
        .append(editedMarkup)
        .append(edited)
