...
public class TexMECSImporter {
  ...
  public TAGDocumentDAO importTexMECS(String input) throws TexMECSSyntaxError {
    CharStream antlrInputStream = CharStreams.fromString(input);
    return importTexMECS(antlrInputStream);
  }

  public TAGDocumentDAO importTexMECS(InputStream input) throws TexMECSSyntaxError {
    try {
  ...
  private TAGDocumentDAO importTexMECS(CharStream antlrInputStream) {
    ...
    TAGDocumentDAO document = listener.getDocument();
  ...
  private void handleMarkupDominance(TAGDocumentDAO document) {
    List<TAGMarkupDAO> markupList = document.getMarkupStream().collect(Collectors.toList());
    for (int i = 0; i < markupList.size() - 1; i++) {
      TAGMarkupDAO first = markupList.get(i);
      TAGMarkupDAO second = markupList.get(i + 1);
