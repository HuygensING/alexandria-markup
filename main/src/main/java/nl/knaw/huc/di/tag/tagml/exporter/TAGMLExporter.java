...
public class TAGMLExporter extends TAGExporter {
  ...
  public String asTAGML(TAGDocument document) {
    ...
    handleOpenTextVariation(textVariationStates, tagmlBuilder);
    handleOpenMarkup(stateRef, tagmlBuilder);
    return tagmlBuilder
        .toString()
        .replaceAll(" +", " ")
  ...
  private void handleTextNode(
    ...
    document
        .getMarkupStreamForTextNode(textNode)
        .forEach(
            mw -> {
              Long id = mw.getDbId();
              markupIds.add(id);
    ...
    boolean showText = view.textIsRelevant(markupIds);

    if (needsDivider(textNode)) {
      tagmlBuilder.append(DIVIDER);
    }

    TextVariationState variationState = textVariationStates.peek();
    if (variationState.isFirstNodeAfterConvergence(textNode)) {
    ...
    toClose.forEach(
        markupId -> {
          String tentativeCloseTag = state.closeTags.get(markupId).toString();
          String closeTag =
              addSuspendPrefixIfRequired(
                  tentativeCloseTag, markupId, discontinuousMarkupTextNodesToHandle);
    ...
    toOpen.forEach(
        markupId -> {
          String tentativeOpenTag = state.openTags.get(markupId).toString();
          String openTag =
              addResumePrefixIfRequired(
                  tentativeOpenTag, markupId, discontinuousMarkupTextNodesToHandle);
    ...
    if (variationState.isBranchStartNode(textNode)) {
    ...
    state.lastTextNodeId = textNode.getDbId();
    //        LOG.debug("TAGML={}\n", tagmlBuilder);
  }

  private void handleOpenTextVariation(
      Deque<TextVariationState> textVariationStates, StringBuilder tagmlBuilder) {
  ...
  private void handleOpenMarkup(
      AtomicReference<ExporterState> stateRef, StringBuilder tagmlBuilder) {
