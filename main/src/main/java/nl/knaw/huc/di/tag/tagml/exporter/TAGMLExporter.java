...
public class TAGMLExporter extends TAGExporter {
  ...
  private void processTextNode(
    ...
    if (!processedNodes.contains(nodeToProcess)) {
      ...
      AtomicReference<Set<String>> closestLayers = new AtomicReference<>(new HashSet<>());
      document
          .getMarkupStreamForTextNode(nodeToProcess)
          .forEach(
              mw -> {
                if (mw.getTextNodeStream().count() == 1) {
                  closestLayers.set(mw.getLayers());
                }
                Long id = mw.getDbId();
                markupIds.add(id);
      ...
      Set<String> textLayers = closestLayers.get();
      ...
      if (variationState.isBranchStartNode(nodeToProcess)) {
        // this node starts a new branch of the current text variation
        stateRef.set(variationState.getStartState());
        variationState.incrementBranchesStarted();
      }

      if (textLayers.isEmpty() || !view.filterRelevantLayers(textLayers).isEmpty()) {
