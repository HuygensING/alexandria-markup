...
/*
 * validations handled by this Validator, that are not implemented by the parser
 * invalid situations:
 * - suspended markup that is not resumed
 * - resuming markup that has not been suspended
 * - close tags without corresponding open tags
 * - open tags without corresponding close tags
 * - overlap within a layer
 * - reuse of an :id
 * - using a layer before it has been added
 * - markup that was opened before a branch is closed in that branch
 * - markup opened in a branch is closed after that branch
 * - markup tags without a markup name
 * - using a namespace that has not been defined
 * - text before the root open tag
 * - text or markup after the root close tag
 * - suspending the root markup
 * - root markup is milestone
 * - leaving out the layer identifier in the close tag when it is ambiguous which open tag this close tag corresponds with
 * - There is a discrepancy in suspended markup between branches
 * - There is an open markup discrepancy between the branches
 * - no text between corresponding suspend and resume tags
 * -
 */
...
public class TAGMLValidator extends TAGMLParserBaseListener {
  private static final Logger LOG = LoggerFactory.getLogger(TAGMLValidator.class);
  ...
  public TAGMLValidator(ErrorListener errorListener) {
    this.errorListener = errorListener;
    this.textVariationStateStack.push(new TextVariationState());
  }

  public class State {
    public Map<String, Deque<String>> openMarkup = new HashMap<>();
    public Map<String, Deque<String>> suspendedMarkup = new HashMap();
    public Deque<TAGMarkup> allOpenMarkup = new ArrayDeque<>();
    public String rootMarkup = "";
    ...
    public State copy() {
      ...
      copy.rootMarkup = rootMarkup;
      copy.eof = eof;
      return copy;
    }

    public boolean rootMarkupIsSet() {
      return !rootMarkup.isEmpty();
    }
  }
  ...
  private void checkEOF(final ParserRuleContext ctx) {
    if (state.eof) {
      errorListener.addBreakingError(
          ...
          errorPrefix(ctx), state.rootMarkup);
    }
  }
