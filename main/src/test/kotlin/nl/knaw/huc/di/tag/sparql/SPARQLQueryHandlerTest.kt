...
class SPARQLQueryHandlerTest : AlexandriaBaseStoreTest() {
    ...
    fun testSPARQLQuerySelect() {
        val body = "[x>[q>and what is the use of a book,<-q] thought Alice[+q>without pictures or conversation?<q]<x]"
    ...
    fun testSPARQLQueryDescribe() {
        val body = "[l>[w>Just<w] [w>some<w] [w>words<w]<l]"
        val tagml = addTAGMLHeader(body)
        runInStoreTransaction { store: TAGStore ->
            val alice = TAGMLImporter(store).importTAGML(tagml)
            val h = SPARQLQueryHandler(alice)
    ...
    fun testSPARQLQueryConstruct() {
        val body = "[l>[person>John<person] went to [country>Spain<country], [person>Rachel<person] went to [country>Peru<country]<l]"
        val tagml = addTAGMLHeader(body)
        runInStoreTransaction { store: TAGStore ->
            val alice = TAGMLImporter(store).importTAGML(tagml)
            val h = SPARQLQueryHandler(alice)
