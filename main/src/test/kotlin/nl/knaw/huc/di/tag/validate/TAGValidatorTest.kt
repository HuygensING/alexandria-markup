...
class TAGValidatorTest : TAGBaseStoreTest() {
    ...
    fun testSimpleTAGMLValidation2() {
        val tagML = "$DUMMY_HEADER[tagml>[l>test [w>word<w]<l]<tagml]"
        val schemaYAML = """
            |$:
            |  tagml:
            |    - l:
    ...
    fun testSimpleTAGMLValidation4() {
        val tagML = "$DUMMY_HEADER[tagml|+A,+B,+C>[l|A>[c|C>test<c] [b|B>[w|A>word<w]<b]<l]<tagml]"
        val schemaYAML = """
            |A:
            |  tagml:
            |    - chapter:
    ...
    fun testSimpleTAGMLValidation3() {
        val tagML = "$DUMMY_HEADER[tagml|+A,+B>[a|A>The rain [b|B>in [aa|A>Spain<aa] falls [bb|B>mainly<bb] on the plain.<b]<a]<tagml]"
        val schemaYAML = """
            |A:
            |  tagml:
            |    - a:
