...
class TAGViewTest : AlexandriaBaseStoreTest() {
    ...
    fun testDefaultLayerIsAlwaysIncludedInInclusiveLayerView() {
        val body = "[tagml>[layerdef|+A,+B>[x|A>C'est [x|B>combien<x|A], cette [b|A>six<b|A]<x|B] <|saucissons|croissants|>-ci?<layerdef]<tagml]"
    ...
    fun testLayerMarkupCombinationInView() {
        val body = "[tagml>[layerdef|+A,+B>[x|A>C'est [x|B>combien<x|A], cette [b|A>six<b|A]<x|B] saucissons-ci?<layerdef]<tagml]"
        val tagml = addTAGMLHeader(body)
        val viewJson = """
            |{
            |  "includeLayers" : ["A"],
    ...
    fun testFilterRelevantMarkup0() {
        runInStoreTransaction { store: TAGStore ->
            ...
            val document1 = importer.importTAGML(addTAGMLHeader("[tagml|+L1,+L2>[a|L1>a[b|L2>b[c|L1>c[d|L2>da<c]b<d]c<a]d<b]<tagml]"))
    ...
    fun testFilterRelevantMarkup() {
        runInStoreTransaction { store: TAGStore ->
            ...
            val document1 = importer.importLMNL(addTAGMLHeader("[a}a[b}b[c}c[d}da{a]b{b]c{c]d{d]"))
    ...
    private fun createNewMarkup(document: TAGDocument, tagName: String, store: TAGStore): Long =
            store.createMarkup(document, tagName).dbId

    private fun createNewMarkup(document: TAGDocument, tagName: String, layer: String, store: TAGStore): Long {
