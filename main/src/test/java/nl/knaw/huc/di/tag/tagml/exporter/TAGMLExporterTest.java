...
public class TAGMLExporterTest extends TAGBaseStoreTest {
  ...
  public void testNonLinearityWith3Branches() {
    String tagML =
        DUMMY_HEADER
            + "[tagml>[layerdef|+A,+B>[l|A>Et voilà que de la <|sombre|jolie|miserable|> [x|B>surface<x|B] d'un étang s'élève un cygne<l|A]<layerdef|A,B]<tagml]";
    assertTAGMLOutIsIn(tagML);
  }
  ...
  public void testBrulez() {
    String tagmlIn =
        DUMMY_HEADER
            + "[doc source=\"typescript-tg_lhhs_107\">\n"
  ...
  public void testSimpleExampleWithMarkupAndTwoLayers() {
    String tagmlIn = DUMMY_HEADER + "[x|+P,+T>[a|T>Donald [b|P>likes<a|T] Vladimir<b|P]<x|P,T]";
    assertTAGMLOutIsIn(tagmlIn);
  }

  @Test
  public void testEscapedCharacters() {
    String tagmlIn = DUMMY_HEADER + "[line>\\[, \\< and \\\\ need to be escaped.<line]";
    assertTAGMLOutIsIn(tagmlIn);
  }

  @Test
  public void testMarkedUpText() {
    String tagmlIn = DUMMY_HEADER + "[line>The rain in Spain falls mainly on the plain.<line]";
    assertTAGMLOutIsIn(tagmlIn);
  }

  @Test
  public void testLinearAnnotatedText() {
    String tagmlIn = DUMMY_HEADER + "[a>I've got a [b>bad<b] feeling about [c>this<c].<a]";
    assertTAGMLOutIsIn(tagmlIn);
  }
  ...
  public void testNonLinearAnnotatedText() {
    String tagmlIn =
        DUMMY_HEADER
            + "[a>I've got a <|[sic>very [b>bad<b]<sic]|[corr>exceptionally good<corr]|> feeling about [c>this<c].<a]";
    assertTAGMLOutIsIn(tagmlIn);
  }
  ...
  public void testOptionalTags() {
    String tagmlIn = DUMMY_HEADER + "[l>This [?w>word<?w] is optional.<l]";
    assertTAGMLOutIsIn(tagmlIn);
  }
  ...
  public void testDiscontinuity() {
    String tagmlIn =
        DUMMY_HEADER
            + "[x>[q>and what is the use of a book,<-q] thought Alice[+q>without pictures or conversation?<q]<x]";
    assertTAGMLOutIsIn(tagmlIn);
  }
  ...
  public void testNonLinearityAndLayers1() {
    String tagML =
        DUMMY_HEADER
            + "[tagml>[layerdef|+A,+B>[l|A>Et voilà que de la <|sombre|jolie|> [x|B>surface<x|B] d'un étang s'élève un cygne<l|A]<layerdef|A,B]<tagml]";
    assertTAGMLOutIsIn(tagML);
  }
  ...
  public void testNonLinearityAndLayers2() {
    String tagML =
        DUMMY_HEADER
            + "[a>[tagml|+A,+B>[l|A>Et voilà que de la <|[w|A>sombre<w|A]|[w|B>jolie<w|B]|> [x|B>surface<x|B] d'un étang s'élève un cygne<l|A]<tagml|A,B]<a]";
    assertTAGMLOutIsIn(tagML);
  }

  @Test
  public void testSingleQuotedStringAnnotation() {
    String tagML = DUMMY_HEADER + "[tagml author='me'>test<tagml]";
    assertTAGMLOutIsIn(tagML);
  }

  @Test
  public void testDoubleQuotedStringAnnotation() {
    String tagML = DUMMY_HEADER + "[tagml author=\"you\">test<tagml]";
    String out = parseAndExport(tagML);
    String expected = DUMMY_HEADER + "[tagml author='you'>test<tagml]";
    assertThat(out).isEqualTo(expected);
  }

  @Test
  public void testStringAnnotationWithEscapedQuotes() {
    String tagML = DUMMY_HEADER + "[tagml author=\"John \\\"Nickname\\\" O'Neill\">test<tagml]";
    String out = parseAndExport(tagML);
    String expected = DUMMY_HEADER + "[tagml author='John \"Nickname\" O\\'Neill'>test<tagml]";
    assertThat(out).isEqualTo(expected);
  }

  @Test
  public void testStringListAnnotation() {
    String tagML = DUMMY_HEADER + "[tagml author=['me','you']>test<tagml]";
    assertTAGMLOutIsIn(tagML);
  }

  @Test
  public void testNumberListAnnotation() {
    String tagML = DUMMY_HEADER + "[tagml odd=[1,3,5]>test<tagml]";
    assertTAGMLOutIsIn(tagML);
  }

  @Test
  public void testBooleanListAnnotation() {
    String tagML = DUMMY_HEADER + "[tagml b=[true,false]>test<tagml]";
    assertTAGMLOutIsIn(tagML);
  }
  ...
  public void testNestedAnnotation() {
    String tagML =
        DUMMY_HEADER
            + "[tagml author={first='John' last='Doe' alias=[{first='Jack' last='White'},{first='Ano' last='Nymous'}]}>test<tagml]";
    assertTAGMLOutIsIn(tagML);
  }
    ...
    runInStore(
        store -> {
          TAGDocument document =
              store.runInTransaction(() -> new TAGMLImporter(store).importTAGML(tagmlIn));
          String tagml =
