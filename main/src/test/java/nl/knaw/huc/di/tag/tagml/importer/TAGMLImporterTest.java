...
public class TAGMLImporterTest extends TAGBaseStoreTest {
        ...
        store -> {
          try {
            ...
            assertThat(customError.range)
                .isEqualTo(new Range(new Position(4, 1), new Position(4, 20)));
          }
        });
  }
  ...
  public void testRD207() throws IOException {
    String tagML =
        FileUtils.readFileToString(new File("data/tagml/CRLR_GR_MS1H16d_ES.tagml"), "UTF-8");
    String view =
        FileUtils.readFileToString(new File("data/tagml/view-stage2-layer.json"), "UTF-8");
    runInStore(
        store -> {
  ...
  public void testLayerIdentifiersAreRequiredInEndTagsWhenThereIsAmbiguity() {
    ...
    final String expectedErrors =
        "line 10:1 : There are multiple start-tags that can correspond with end-tag <p]; add layer information to the end-tag to solve this ambiguity.\n"
            + "parsing aborted!";
  ...
  public void testMissingOpenTagLeadsToError() {
    ...
    String expectedErrors = "line 2:17 : Close tag <t] found without corresponding open tag.";
    runInStoreTransaction(store -> parseWithExpectedErrors(tagML, expectedErrors));
  }
  ...
  public void testSimpleTAGML() {
    ...
    runInStoreTransaction(
        store -> {
          ...
          final List<TAGMarkup> markupForTextNode =
              document.getMarkupStreamForTextNode(textNode).collect(toList());
  ...
  public void testMissingEndTagThrowsTAGMLSyntaxError() {
    ...
    String expectedErrors = "line 2:1 : Missing close tag(s) for: [line>";
  ...
  public void testMissingOpenTagThrowsTAGMLSyntaxError() {
    ...
    String expectedErrors =
        "line 1:33 : No text allowed here, the root markup must be started first.\n"
            + "parsing aborted!";
    parseWithExpectedErrors(tagML, expectedErrors);
  }
  ...
  public void testOptionalMarkup() {
    ...
    runInStoreTransaction(
        store -> {
          ...
          List<TAGMarkup> TAGMarkups =
              document.getMarkupStreamForTextNode(always).collect(toList());
  ...
  public void testUseLayersForSelfOverlap() {
    ...
    runInStoreTransaction(
        store -> {
          ...
          TAGMarkup phr0 = TAGMarkups.get(1);
          assertThat(phr0).hasTag("phr");
          List<TAGTextNode> textNodes0 =
              document.getTextNodeStreamForMarkupInLayer(phr0, "p1").collect(toList());
          assertThat(textNodes0).extracting("text").containsExactly("word2 ", "word3");

          TAGMarkup phr1 = TAGMarkups.get(2);
          assertThat(phr0).hasTag("phr");
          List<TAGTextNode> textNodes1 =
              document.getTextNodeStreamForMarkupInLayer(phr1, "p2").collect(toList());
              ...
              store.runInTransaction(
                  () -> {
                    ...
                    List<AnnotationInfo> annotationInfos =
                        markup.getAnnotationStream().collect(toList());
                    ...
                    //      List<TAGTextNode> annotationTextNodes =
                    // annotationPrimes.getDocument().getTextNodeStream().collect(toList());
  ...
  public void testJustAMilestoneIsNotValidTAGML() {
    ...
    String expectedErrors = "line 2:1 : The root markup cannot be a milestone tag.";
    parseWithExpectedErrors(tagML, expectedErrors);
  }
  ...
  private void parseWithExpectedErrors(final String body, final String expectedErrors) {
    String tagML = addTAGMLHeader(body);
    runInStoreTransaction(
        store -> {
          try {
            TAGDocument document = parseTAGML(tagML, store);
  ...
  private TAGDocument parseTAGML(final String tagML, final TAGStore store) {
    ...
    if (!trimmedTagML.startsWith("[!schema")) {
      trimmedTagML = "[!schema file:///tmp/schema]\n" + trimmedTagML;
    }
