...
public class TextGraphTest {
  ...
  public void testConstruction() {
    // [tagml>[a|+a>[name|+ner>J'onn<name|ner] [b|+b>craves<a|a]
    // [name|ner>Oreos<name|ner]<b|b]<tagml]
    Long root = newNode();
    ...
    assertThat(defaultTextIds)
        .containsExactly(textJonn, textSpace1, textCraves, textSpace2, textOreos);
    ...
    List<Long> markupForJonnNER =
        tg.getMarkupIdStreamForTextNodeId(textJonn, layerNER).collect(toList());
    ...
    List<Long> textForDefault =
        tg.getTextNodeIdStreamForMarkupIdInLayer(markupTagml, "").collect(toList());
    assertThat(textForDefault)
        .containsExactly(textJonn, textSpace1, textCraves, textSpace2, textOreos);

    List<Long> textForA =
        tg.getTextNodeIdStreamForMarkupIdInLayer(markupTagml, layerA).collect(toList());
    assertThat(textForA).containsExactly(textJonn, textSpace1, textCraves);

    List<Long> textForB =
        tg.getTextNodeIdStreamForMarkupIdInLayer(markupTagml, layerB).collect(toList());
    assertThat(textForB).containsExactly(textCraves, textSpace2, textOreos);

    List<Long> textForNER =
        tg.getTextNodeIdStreamForMarkupIdInLayer(markupTagml, layerNER).collect(toList());
  ...
  public void testNesting() {
    ...
    List<Long> textForPhr1 =
        tg.getTextNodeIdStreamForMarkupIdInLayer(markupPhr1, layerDefault).collect(toList());
