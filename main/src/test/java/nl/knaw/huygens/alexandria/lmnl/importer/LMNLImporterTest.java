...
public class LMNLImporterTest extends AlexandriaBaseStoreTest {
  ...
  public void testLexingComplex() throws LMNLSyntaxError {
    runInStoreTransaction(
        store -> {
          ...
          AnnotationInfo source =
              simpleAnnotation("source") /*.addAnnotation(date).addAnnotation(title)*/;

          AnnotationInfo name = simpleAnnotation("name", "Robert Frost");
          AnnotationInfo dates = simpleAnnotation("dates", "1874-1963");
          AnnotationInfo author =
              simpleAnnotation("author") /*.addAnnotation(name).addAnnotation(dates)*/;

          TAGMarkup excerpt =
              store
                  .createMarkup(expected, "excerpt")
          ...
          TAGMarkup l1 =
              store.createMarkup(expected, "l") /*.addTextNode(tn01)*/.addAnnotation(n144);

          AnnotationInfo n145 = simpleAnnotation("n", "145");
          TAGMarkup l2 =
              store
                  .createMarkup(expected, "l")
                  .setFirstAndLastTextNode(tn03, tn07)
                  .addAnnotation(n145);

          AnnotationInfo n146 = simpleAnnotation("n", "146");
          TAGMarkup l3 =
              store.createMarkup(expected, "l") /*.addTextNode(tn09)*/.addAnnotation(n146);

          expected
              .addTextNode(tn00, null)
  ...
  public void testLMNLOzymandias() throws IOException, LMNLSyntaxError {
    ...
    runInStoreTransaction(
        store -> {
          ...
          assertThat(tagml)
              .startsWith(
                  "[sonneteer [id}ozymandias{] [encoding [resp}ebeshero{] [resp}wap{]]}"); // annotations from sonneteer endtag moved to start tag
          assertThat(tagml)
              .contains(
                  "[meta [author}Percy Bysshe Shelley{] [title}Ozymandias{]]"); // anonymous markup
          // compareTAGML(pathname, actual);
  ...
  public void testAnnotationTextWithRanges() throws LMNLSyntaxError {
    runInStoreTransaction(
        store -> {
          ...
          //      TAGMarkup am1 = store.createMarkup(annotationDocument,
          // "type")/*.addTextNode(at2)*/;
  ...
  public void testAnnotationTextInAnnotationWithRanges() throws LMNLSyntaxError {
    ...
    runInStoreTransaction(
        store -> {
          ...
          //      TAGMarkup ar11 = store.createMarkup(annotationDocument,
          // "ra11")/*.addTextNode(at1)*/;
          //      TAGMarkup ar12 = store.createMarkup(annotationDocument,
          // "ra12")/*.addTextNode(at1)*/;
          //      annotationDocument
          //          .addTextNode(at1)
          //          .addMarkup(ar11)
          //          .addMarkup(ar12);
    ...
    runInStoreTransaction(
        store -> {
          ...
          try {
            ...
            assertThat(e.getMessage())
                .contains("Closing tag {b] found without corresponding open tag.");
          }
        });
  }
  ...
  private void assertActualMatchesExpected(
      TAGDocument actual, TAGDocument expected, final TAGStore store) {
    List<TAGMarkup> actualMarkupList = actual.getMarkupStream().collect(toList());
