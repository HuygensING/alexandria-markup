...
public class AnnotationInfoAssert
    extends AbstractObjectAssert<AnnotationInfoAssert, AnnotationInfo> {
  ...
  private AnnotationInfoAssert assertAnnotationTypeIs(final AnnotationType expectedAnnotationType) {
    ...
    if (!actual.getType().equals(expectedAnnotationType)) {
      failWithMessage(
          "Expected annotation's type to be <%s> but was <%s>",
          expectedAnnotationType.name(), actual.getType());
    }
    return myself;
  }
}
