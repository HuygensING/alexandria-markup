...
class TAGViewTest : AlexandriaBaseStoreTest() {
  ...
  @Test
  fun text_is_exclusive_to_layer_of_direct_markup() {
    val json = ("""{
                |"includeLayers" : ["A"]
                |}""".trimMargin())
    val tagml = "[TAGML|+A,+B>[w|A>To be<w], [w|B>or<w] not to be.<TAGML]" // "or" is exclusive to layer B
    val expected = "[TAGML|+A,+B>[w|A>To be<w|A], not to be.<TAGML|A,B]" // normalize whitespace
    runInStore { store: TAGStore ->
      val tagViewFactory = TAGViewFactory(store)
      val view = tagViewFactory.fromJsonString(json)
